...
int main() {
  ...
  for (int i = 0; i < repeatQuantity; i++) {
    for (int j = 0; j < repeatQuantity - i; j++) {
      printf("\n------------------------------\n");
      printf("%dº repetição %dº comparação -> compara %d com %d\n", i + 1,
             j + 1, list[j], list[j + 1]);

      // 1 comparação
      if (list[j] > list[j + 1]) {

        printf("\nComo %d é maior que %d, troca os valores\n", list[j],
               list[j + 1]);
        printf("\n1º %d é colocado em uma variável auxiliar", list[j]);
        printf("\n2º %d é colocado no lugar de %d", list[j + 1], list[j]);
        printf("\n3º auxiliar contendo %d é colocado no lugar de %d", list[j], list[j + 1]);
        printf("\n------------------------------\n");
